...
const RAV_MESSER_OAUTH_TIMEOUT_MS = 10000;
const RAV_MESSER_SUBSCRIBER_TIMEOUT_MS = 10000;
...
const SENSITIVE_DIAGNOSTIC_KEY_PATTERN =
  /authorization|credential|email|name|secret|token/i;
...
function sanitizeDiagnosticValue(
  ...
  if (Array.isArray(value)) {
    return value
      .slice(0, 20)
      .map((item) =>
        sanitizeDiagnosticValue(item, sensitiveValues, depth + 1),
      );
  }
  ...
  return Object.fromEntries(
    Object.entries(value)
      .slice(0, 20)
      .map(([key, nestedValue]) => [
        sanitizeDiagnosticValue(key, sensitiveValues),
...
async function getAccessToken(config, forceRefresh = false) {
  ...
  const response = await fetchWithTimeout(
    `${RAV_MESSER_API_URL}/oauth/token`,
    {
      method: "POST",
      headers: {
...
function createSubscriber(config, accessToken, name, email) {
  return fetchWithTimeout(
    `${RAV_MESSER_API_URL}/subscribers`,
    {
      method: "POST",
      headers: {
        Accept: "application/json",
    ...
    RAV_MESSER_SUBSCRIBER_TIMEOUT_MS,
  );
}
...
async function processSubscription(payload, requestId = "unknown") {
  ...
  try {
    ...
    console.info("Rav Messer subscriber response diagnostics", {
      keys:
      ...
      duplicateType: typeof result?.duplicate,
      createdId: result?.createdId,
      createdIdType: typeof result?.createdId,
      id: sanitizeDiagnosticValue(
        result?.id,
        diagnosticSensitiveValues,
      ),
      ...
      errors: sanitizeDiagnosticValue(
        result?.errors,
        diagnosticSensitiveValues,
      ),
    });
